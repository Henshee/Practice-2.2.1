...
public class UserDaoImp implements UserDao {
   ...
   public User findOwner(String car_name, String car_series) {
      TypedQuery<Car> findCarQuery = sessionFactory.getCurrentSession().createNativeQuery("from Car where name = : car_name and series = :car_series")
      ...
      if (!findCarList.isEmpty()) {
         ...
         User FindUser = ListUser.stream()
                 .filter(user -> user.getCars().equals(findCar))
                 .findAny()
                 .orElse(null);
         return FindUser;
      }
